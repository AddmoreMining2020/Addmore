...
#if defined(NDEBUG)
# error "Addmore Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in args) are considered zero fee (for relaying, mining and transaction creation) */
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    if (!nPrevHeight && nPrevHeight == 0) {
        return 500000 * COIN;
    } else if (nPrevHeight >= 1 && nPrevHeight < 2000) {
        return 1 * COIN;
    } else if (nPrevHeight >= 2000 && nPrevHeight < 5000000) {
        return 10 * COIN;
    }
...
{
    if (nHeight >= 5000 && nHeight < 1000000) {
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // ADDMORE CORE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ADDMORE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ADDMORE): couldn't find masternode or superblock payments"),
    ...
    if (founderPayment > 0) {
        ...
        if (!FounderPaid) {
            return state.DoS(0, error("ConnectBlock(ADDMORE): no founder reward"), REJECT_INVALID, "no-founder-reward");
        }
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ADDMORE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ADDMORE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ADDMORE): spork is off, skipping transaction locking checks\n");
    }
