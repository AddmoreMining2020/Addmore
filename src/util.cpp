...
const char * const BITCOIN_CONF_FILENAME = "addmore.conf";
const char * const BITCOIN_PID_FILENAME = "addmored.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "addmore" is a composite category enabling all Addmore-related debug output
            if(ptrCategory->count(string("addmore"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Addmore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Addmore
    // Mac: ~/Library/Application Support/Addmore
    // Unix: ~/.addmore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Addmore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Addmore";
#else
    // Unix
    return pathRet / ".addmore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty addmore.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override addmore.conf
