    ...
    if(ui->radioListMode->isChecked()){
        ...
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++){
            ...
            if (model->isLockedCoin(uint256S(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt())){
                ...
                item->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/" + theme + "/lock-closed"));
            }
            updateLabelLocked();
        }
        ui->treeWidget->setEnabled(true);
...
void CoinControlDialog::lockCoin()
{
    ...
    contextMenuItem->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/" + theme + "/lock-closed"));
    updateLabelLocked();
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::ADD;
    ...
    QString toolTip4 = tr("Can vary +/- %1 arg(s) per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show addmore address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            if (model->isLockedCoin(txhash, out.i))
            {
                ...
                itemOutput->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/" + theme + "/lock-closed"));
            }
