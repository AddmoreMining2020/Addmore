...
// CRC32 instruction is used a lot as it is extremely fast on low-power ADD
...
//   - on ADDv8: use gcc -march=native or -march=armv8-a+crypto+crc to enable
...
/* Use -march=armv8-a+crypto on ADDv8 to use crypto extensions */
/* Use -maes on x86_64 to use AES-NI */
#if defined(RF_NOASM) || (!defined(__aarch64__) || !defined(__ADD_FEATURE_CRYPTO)) && (!defined(__x86_64__) || !defined(__AES__))
...
#endif // (!defined(__aarch64__) || !defined(__ADD_FEATURE_CRYPTO)) && (!defined(__x86_64__) || !defined(__AES__))
...
#if defined(__aarch64__) && defined(__ADD_FEATURE_CRYPTO)
    asm volatile(
        "ld1   {v0.16b},[%0]        \n"
...
#elif defined(__i386__)||defined(__ADD_ARCH_7A__)
#define RF_UNALIGNED_LE32
#endif
...
// build with -mcpu=cortex-a53+crc to enable native CRC instruction on ADD
static inline uint32_t rf_crc32_32(uint32_t crc, uint32_t msg) {
#if defined(__aarch64__) && defined(__ADD_FEATURE_CRC32)
  asm("crc32w %w0,%w0,%w1\n":"+r"(crc):"r"(msg));
#else
  crc=crc^msg;
...
#if !defined(__ADD_ARCH_8A) && !defined(__AARCH64EL__) && !defined(x86_64)
  bits&=63;
#endif
...
#if defined(__ADD_ARCH_8A) || defined(__AARCH64EL__)
...
#if defined(__x86_64__) || defined(__aarch64__) || defined(__ADD_ARCH_7A__)
  uint32_t t0, t1, t2;
