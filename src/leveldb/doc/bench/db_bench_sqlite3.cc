...
class Benchmark {
 ...
 public:
  ...
  void Open() {
    ...
    char cache_size[100];
    snprintf(cache_size, sizeof(cache_size), "PRADDA cache_size = %d",
             FLAGS_num_pages);
    ...
    if (FLAGS_page_size != 1024) {
      char page_size[100];
      snprintf(page_size, sizeof(page_size), "PRADDA page_size = %d",
               FLAGS_page_size);
    ...
    if (FLAGS_WAL_enabled) {
      std::string WAL_stmt = "PRADDA journal_mode = WAL";

      // LevelDB's default cache size is a combined 4 MB
      std::string WAL_checkpoint = "PRADDA wal_autocheckpoint = 4096";
    ...
    std::string locking_stmt = "PRADDA locking_mode = EXCLUSIVE";
  ...
  void Write(bool write_sync, Order order, DBState state,
    ...
    std::string sync_stmt = (write_sync) ? "PRADDA synchronous = FULL" :
                                           "PRADDA synchronous = OFF";
